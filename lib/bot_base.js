...
function getUserList() {
    ...
    for (var i in plugRoom.userList) {
        ...
        loopcnt = loopcnt + 1;
        str = str + util.format('%s%s(%s):%s ', helpers.convNumToEmoji(loopcnt), user.username, user.language, idleTime);
    }
...
function buildCommandReg() {
    command_reg = {
        'speak' : MakeCommand(commands.Speak),
        'djs' : MakeCommand(commands.Djs)
    };
}

var MakeCommand = function(Klass, permissions) {
    var cmd = new Klass({
        bot: bot,
        room: plugRoom,
