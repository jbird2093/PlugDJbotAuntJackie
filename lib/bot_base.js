        ...
        client.query(cmd, function (err, results, fields) {
            ...
            if (results) {
                ...
                for (i = 0; i < results.length; i++) {
                    ...
                    var plays = parseInt(results[i]['plays']);
                    var woots = parseInt(results[i]['woots']);
                    var mehs = parseInt(results[i]['mehs']);
                    var grabs = parseInt(results[i]['grabs']);
...
function djAdvance(data) {
    ...
    if (config.pgreets) {
        botSpeak('chat', util.format('%s played %s by: %s Votes: %d:+1: %d:-1: %d:heart:', currentSong.djName, currentSong.title, currentSong.artist, currentSong.score.woots, currentSong.score.mehs, currentSong.score.grabs));
    }
    ...
    switch (commandObj.command) {
        ...
        case 'stagedive':
            if (userid === currentSong.djId) {
                plugRoom.getUser(currentSong.djId).escort = true;
        ...
        case 'myfavs':
            ...
            //        bot.changeRoomOptions(false, true, 1, config.deckPlays);
            //    } else {
