...
var TrackEnforce = require("./track_enforce").TrackEnforce;
...
function getPersonalGreet(user) {
    var min = 0;
    var max = null;
    if (config.usedb && config.pgreets) {
...
function onRoomJoin(data) {
    ...
    MakeCommand(commands.Playlists)
...
function onChat(chatItem) {
    ...
    if (msgText.match(/!!/)) {
        var historyItem = plugRoom.getHistoryItem();
        if (historyItem) {
...
var configureCommands = function () {
    ...
    commandHandler
        ...
        .on('playlists', MakeCommand(commands.Playlists))
        ...
        .on('songlimit', MakeCommand(commands.SetSongLimit))
        .on('songtimer', MakeCommand(commands.ToggleSongTimer))
        .on('afkcheck', MakeCommand(commands.AfkCheck))
        .on('setafk', MakeCommand(commands.SetAFK))
        .on('setgames', MakeCommand(commands.SetGames))
        .on('setgreets', MakeCommand(commands.SetGreets))
    ...
    if (config.usedb) {
        ...
        .on('funnyme', MakeCommand(commands.FunnyMe))
