...
(function () {
    ...
    var PERMISSION_ANYONE = 1,
        ...
        PERMISSION_SELF = 32,
        PERMISSION_BIG_KID = PERMISSION_STAFF | PERMISSION_OWNER | PERMISSION_MASTER;
    ...
    _.extend(BaseCommand.prototype, {
        ...
        // anyone can do a command by default
        // set this in your command to control access
    ...
    var DJCommand = BaseCommand.extend({
        PERMISSION_LEVEL: PERMISSION_BIG_KID,
    ...
    var SnagCommand = BaseCommand.extend({
        PERMISSION_LEVEL: PERMISSION_OWNER | PERMISSION_MASTER,
    ...
    var getPlaylists = BaseCommand.extend({
        PERMISSION_LEVEL: PERMISSION_OWNER | PERMISSION_MASTER,

        _execute: function (cmd) {
            this.bot.getPlaylists(function (playlists) {
    ...
    var SmokeCommand = BaseCommand.extend({
        ...
        getSmoke: function () {
            var self = this;
            var text = ":herb:SMOKE IT IF YOU GOT IT!:herb:";
            if (this.options.db) {
                ...
                this.options.db.query(cmdget, function (err, results, fields) {
                    ...
                    if (results) {
                        self.speak(text + ' ' + results[0].text);
                    }
                });
            } else {
                self.speak(text);
    ...
    var TeachMeCommand = DBCommand.extend({
        getTeachme: function () {
            var self = this;
            if (config.sexy) {
                var cmdget = "SELECT text FROM " + config.dbprefix + "teachme ORDER BY rand() LIMIT 1";
        ...
        getTopDJs: function () {
            ...
            this.select(cmd, function (results) {
                if (results.length > 0) {
                    ...
                    self.speak(response + 'in ' + config.room + '(since ' + config.openDate + ')');
                } else {
                    self.speak("Sorry no results.");
                }
            });
    ...
    var ThisTrackCommand = DBCommand.extend({
        getTrackStats: function (media, thistrack) {
            ...
            if (title != 'Untitled') {
                this.selectOne({
                    q: "SELECT COUNT(title) AS plays, SUM(woots) AS woots, SUM(mehs) AS mehs, SUM(grabs) AS grabs " +
                ...
                function (row) {
                    if (row.plays > 0) {
                        ...
                        if (!thistrack) {
                            ...
                            response = "This is a track I've not heard yet. (since " + config.openDate + ")";
                        }
                        self.speak(response);
                    }
                });
    ...
    var TopTracksCommand = DBCommand.extend({
        getTopTracks: function () {
            ...
            var limit = 10;
            var response = '';
            var cmd = "SELECT COUNT(title) AS plays, title FROM " + config.dbprefix + "songs WHERE (userid <> '" + config.userid + "') AND (title <> 'Untitled') "
            ...
            this.select(cmd, function (results) {
                if (results.length > 0) {
                    for (i = 0; i < limit; i++) {
                        ...
                        var title = row.title;
                        var plays = row.plays;
                        response = response + util.format('%s%s has %s:musical_note:. ', helpers.convNumToEmoji(i + 1), title, plays);
                    }
                    self.speak(response + ' sorted by :+1: (since ' + config.openDate + ')');
                } else {
                    self.speak("Sorry no results.");
                }
            });
    ...
    var MyFavsCommand = DBCommand.extend({
        getMyFavs: function (userid, username) {
            ...
            var limit = 7;
            var response = '';
            var cmd = "SELECT COUNT(title) AS plays, title FROM " + config.dbprefix + "songs " +
    ...
    var SetSongLimitCommand = BaseCommand.extend({
        ...
        _execute: function (cmd) {
            config.songLimit = cmd.param;
            this.speak('songLimit set to: ' + config.songLimit);
        }
    });

    var ToggleSongTimerCommand = BaseCommand.extend({
        ...
        _execute: function (cmd) {
            config.songTimer = cmd.param;
            this.speak('songTimer set to: ' + config.songTimer);
        }
    });
    ...
    var SetGamesCommand = BaseCommand.extend({
        ...
        isValid: validations.hasBoolParam
    });
    ...
    var SetGreetsCommand = BaseCommand.extend({
        ...
        _execute: function (cmd) {
            ...
            config.greets = map[cmd.param];
            this.speak("reg greets set to: " + config.greets);
        },
        ...
        _execute: function (cmd) {
            ...
            if (cmd.who.isStaff) {
                this.speak("STAFF: " +
                    ...
                    config.botCall + "songtimer (true/false), " +
                    config.botCall + "songlimit X, " +
                    config.botCall + "afkcheck (true/false), " +
                    config.botCall + "setafk X, " +
                    config.botCall + "setgames (true/false), " +
                    config.botCall + "setgreets (true/false), " +
